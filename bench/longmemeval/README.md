...
| `--profile <name>` | `smoke` (1 fixed instance, first session only), `probe` (first session only for a chosen instance), `qwen-smoke` (Qwen 3.6 Plus via OpenRouter), `full-s` (S dataset), `full-m` (M dataset) |
