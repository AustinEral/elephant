...
Fast iteration on prompts — no Docker, no embeddings. Only needs `LLM_PROVIDER`, `LLM_API_KEY`, and `LLM_MODEL`. Valid providers are `anthropic`, `openai`, `gemini`, and `vertex`, where `openai` uses the Responses API and `gemini`/`vertex` use the native `generateContent` API.
